...
public class BTree<T extends Comparable<T>> {
    ...
    protected Deque<Deque<T>> backtrackDeque = new LinkedList<Deque<T>>();
    /**
    ...
    public void insert(T value) {
        ...
        if (root == null) {
            ...
            while (currentNode != null && !wasAdded) {
                ...
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                	back.addFirst(currentNode.keys[(currentNode.getNumberOfKeys())/2]); //add the popped up key to the deque
                	split(currentNode);
        ...
        back.addFirst(value); //add the inserted key to the deque
        backtrackDeque.addFirst(back); //add the deque to the backtrack deque of deques
        ++size;
